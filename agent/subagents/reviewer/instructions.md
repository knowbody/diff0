...
For changes to the diff0 engine or Action, require evidence from both the ordinary repository checks and the deterministic `fixtures/demo-agent` diff0 comparison. Treat a red diff0 verdict as blocking. Investigate a yellow verdict and require the implementer to explain why the drift is acceptable before approval.
