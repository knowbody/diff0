...
/** Egress needed only while the trusted bootstrap installs locked dependencies. */
const BOOTSTRAP_INSTALL_NETWORK_POLICY = {
  allow: ["registry.npmjs.org", "*.npmjs.org", "github.com", "*.githubusercontent.com"],
...
export async function factoryOnSession({ use }: SandboxSessionContext): Promise<void> {
  ...
  try {
    ...
    if (head.exitCode !== 0 || !/^[a-f0-9]{40}$/.test(sha)) {
      throw new Error(`Could not record the fetched default-branch commit for ${FACTORY_REPO}.`);
    }
    await sandbox.writeTextFile({
      content: JSON.stringify({ branch: defaultBranch, sha }),
