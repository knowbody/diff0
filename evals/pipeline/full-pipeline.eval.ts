...
export default defineEval({
  description:
    "A small real work item runs the whole line: all four stations fire in order and the final report names the branch or draft pull request that was delivered. Opt-in: this pushes a real branch to FACTORY_REPO, so run it deliberately against a scratch repository (pnpm agent:eval pipeline/full-pipeline).",
  ...
  async test(t) {
    ...
    for (const station of STATIONS) {
      t.calledTool("run_station", { input: { station } });
    }
