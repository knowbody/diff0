...
5. When the change touches `src/`, `action/`, `prices.json`, or the root package metadata, build first and run diff0 against the deterministic demo agent: `node dist/cli.js run --repo . --app-dir fixtures/demo-agent --base <default-branch> --head HEAD --runs 3 --fail-on regression`. Record the verdict and report location. This is a behavioral regression check, not a replacement for the repository checks.
